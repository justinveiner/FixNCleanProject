...
while (pRun == True):
    ...
    if (menu_choice == "1"):
        ...
        print("\nThis function will assign volunteers to time slots.\n")
        ...
        print("\nVolunteers assigned successfully. Please see 'schedule.xlsx' in the program folder.\n\n")
            ...
            for client in clients[i]:
                for j in range(len(newDict[client].members)):
                    ...
                    message = """From: Fix 'N' Clean Team %s
                           ...
                           """ % (
                    newDict[client].members[j].email, newDict[client].members[j].name, data[i - 1],
